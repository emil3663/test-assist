...
| **"55 Playwright tests"**, "regression suite... 47 tests" | `README.md`, "Testing" section | Counted directly in `tests/smoke.spec.ts` and `tests/regression.spec.ts`: **8 smoke + 41 regression = 49 total**, not 55/47. **False**, and doesn't match `npm run test:regression`'s own comment either — worth checking whether the count was always wrong or drifted. <br><br>**Retracted 2026-09-12 — this finding was wrong.** Measured with `playwright test --list` rather than by counting `test(` calls in the source: **47 regression + 8 smoke = 55**, exactly as `README.md` states. The source count missed `regression.spec.ts:487:9`, one `test()` inside a loop that Playwright collects as seven tests (KS-01–KS-07). The README was never wrong here; the audit method was. The pytest counts in the rows above were separately confirmed and stand. |
