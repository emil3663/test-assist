...
| CAP-14 | `activate()` on a multi-screen virtual desktop | One overlay window per screen, each set to that screen's own geometry - not one window spanning the virtual desktop, which could only ever take one screen's DPR (TA-232) | ✅ |
| CAP-14b | A window's screen geometry after `activate()`, then hide | Captured once at construction as a plain attribute, not re-read from Qt; unchanged by hiding the window | ✅ |
| CAP-15 | A gap between mismatched screens (e.g. different monitor heights) | Not dimmed as if it were selectable area - as of TA-232, because no per-screen overlay window's geometry reaches it at all, not because it is separately clipped out of one window that does | ✅ |
| CAP-16 | A selection spanning a horizontal gap *between* two screens (DSP-08) | Composited side by side, no black band; the true vertical offset between screens is kept. This is the closeable-gap case (`plan_capture()` packs the pieces adjacently) - distinct from TA-231's unequal-height case below, where the missing region cannot be packed away on either axis | ✅ |
| TA-231 | A selection spanning two screens of unequal logical height, extending past the shorter one's edge | During the drag, the extra region (belonging to no screen) is covered by no overlay window, so it is visibly not selectable rather than a surprise only in the exported file. Any part of a composited result a piece never paints stays transparent (`result.fill(Qt.GlobalColor.transparent)`), not black - the recorded decision for that region, see `docs/TA-231.md`. The overlay-visibility half is covered by an automated test; the exact composited geometry for this specific L-shaped case is not (`plan_capture()`'s own axis-packing is unchanged by this fix and out of scope - see `docs/TA-231.md`) | ✅ |
...
| HIS-08 | A recording alongside PNG snapshots | Both appear together in the same date-filtered gallery list | ✅ |
| HIS-09 | A recording's gallery entry | A distinct card, never decoded as a QPixmap | ✅ |
...
| UI-03 | Zoom, Stroke, Arrow, Highlight Fill, Save PNG, Copy, Export JSON | Live in a full-width settings row under the tool row, not the fixed-width right panel | ✅ |
| UI-04 | The settings row at the 960px minimum window width | Fits without clipping or squeezing any control narrower than its own content | ✅ |
| UI-05 | Save PNG among the other settings-row buttons | Stays visually primary (accent colour, taller) rather than becoming just another button in the row | ✅ |
